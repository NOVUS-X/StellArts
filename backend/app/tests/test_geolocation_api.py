...
async def test_geolocation_api(client):
    ...
    login_resp = client.post("api/v1/auth/login", json={"email": "geo@test.com", "password": "Pass123!"})
    token = login_resp.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}

    # Mock the geocode_address method of the global geolocation_service
    with patch("app.services.geolocation.geolocation_service.geocode_address", new_callable=AsyncMock) as mock_geo:
